...
export function DataLibrary({ variant = "default" }: { variant?: "default" | "drawer" }) {
  ...
  const predictTabContent = (
    <Space direction="vertical" size={12} style={{ width: "100%" }}>
      {!compact ? (
        <Paragraph>
          Для таблицы в блоке <Text strong>«предсказать»</Text> выбери источник: числа <Text strong>в блоке</Text>,{" "}
          строка из <Text strong>«Входы»</Text>, либо <Text strong>файл из «Обучение»</Text> (одна строка по номеру или
          весь файл — таблица ответов откроется во «Визуализации»). Для картинок — только вход из библиотеки.
        </Paragraph>
      ) : (
        <Text type="secondary">
          Таблица: блок, «Входы» или файл из «Обучение». Картинки: только сохранённый вход.
        </Text>
      )}
      <Collapse
