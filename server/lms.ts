...
Архив Nodly ночью не похож на музей: сюда не заходят «посмотреть свет». Папка с красной лентой — значит, кто-то уже посчитал риск и решил, что без цифр не разберутся.

В оранжерее **отвалились подписи** у редких ирисов; в логах всплывает метка **Серебряная пыльца**. Говорят, выручить может только пигмент **одного** вида — но какого, если на табличке остались одни измерения?

Дальше — без мистики: только то, что можно сверить по строкам, обучению и проверке.`
      },
      {
        id: "q00_hero",
        type: "media",
...
Допуск к студии: собери цепочку **Iris → обучение → предсказание** и проверь, держит ли модель смысл.`
      ...
      },
      {
        ...
        type: "text",
        body: `### После практики — про «Данные» и досье
...
Из досье: **setosa** здесь — условный «спасительный» вариант; на коротком лепестке модель чаще всего тянется к **Iris-setosa**. Идея для своей схемы (не часть миссии): прогнать несколько предсказаний подряд и отдельным шагом отметить тот случай, где класс совпал с \`Iris-setosa\` — как сигнал «нашли нужный вид».`
      },
