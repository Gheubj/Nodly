...
Ты заходишь в **цифровой Архив Noda** — не как гость на экскурсию, а как **стажёр**, которого впускают к живым делам. Здесь хранятся данные тысяч историй: растения, голоса, изображения, модели. Тебя встречает **Нодус** — хранитель этих историй, спокойный, как детектив-наставник: без пафоса, зато с привычкой всё перепроверять по следам.

**Стажёр, у нас первый вызов.** В **Архиве Ирисов** — то есть в оранжерее редких сортов — сбой: система **перепутала подписи**. Пока табличка не сходится, нельзя понять, у какого именно цветка есть **ключевой пигмент**, который нужен, чтобы остановить вымышленное заражение — **«Серебряную пыльцу»**. Руками перебирать лепестки и чашелистики можно до утра, а времени нет.

Мне нужен помощник, который **быстро поймёт, как работает ИИ**: не «поверить модели на слово», а собрать цепочку **данные → обучение → проверка → предсказание** и увидеть, где цифры лгут, а где держатся.

Дальше Нодус ведёт тебя по фактам — через **Nodly**, наш стол для сборки моделей.`
      },
      {
        id: "q00_hero",
        type: "media",
...
Зал Архива — рабочее место: измерения, графики, журнал, по которому потом **восстанавливают ход расследования**. Нодус любит говорить, что «архив не спит», если не сходятся строки.

Оранжерея — другая зона, та же логика: пока не ясен **класс** цветка, неясно, какой вид даст нужный пигмент против **Серебряной пыльцы**.`
      },
      {
        id: "q00_problem",
        type: "text",
...
Нодус подключит тебя к **Nodly**: таблица Iris и ответ, который можно **перепроверить** цифра к цифре.`
      },
...
Допуск к студии: собери цепочку **Iris → обучение → предсказание** и проверь, держит ли модель смысл.`
      ...
      },
      {
        ...
        type: "text",
        body: `### После первой практики — про «Данные» и досье

В **«Данные»** можно **загрузить свой CSV** и при необходимости явно указать **целевую колонку** с меткой класса.

Если таблиц в проекте ещё нет, подставляется учебный **Iris.csv** — выбери его в **«Обучение»**. Несколько «подозреваемых» цветков удобно завести во **«Входы»**; можно и вводить числа **вручную в блоке предсказания**, без списка во «Входах».

Из досье: **setosa** здесь — тот самый «спасительный» вариант по пигменту; на коротком лепестке модель чаще всего тянется к **Iris-setosa**. Ниже открыта **вторая разработка в Nodly**: там нужно собрать цепочку, которая при обнаружении этого класса выводит сигнал — как в настоящем расследовании.`
      },
      { id: "q_div_09a", type: "divider" },
      {
        ...
        type: "text",
        textScale: "lg",
        body: `## Сцена 10. Вторая разработка: сигнал «нужный вид»

Отдельный стол в **Nodly** — новый черновик дела. Снова подключи **Iris**, обучи модель и сделай предсказание, а затем собери **ветку логики**, о которой говорил Нодус:

после **«Предсказать»** добавь **«если … то»**: в условии сравни **предсказанный класс** с текстом **Iris-setosa**; в ветке «да» повесь **«показать сообщение»** (например: «пигмент найден — оранжерея в безопасности»). Можно завести **несколько строк** во **«Входы»** и прогнать цепочку по очереди — так видно, на каком образце срабатывает сигнал.`
      },
      {
        id: "q09b_studio_setosa",
        type: "studio",
        instruction:
          "Новый проект миссии: Iris → обучение → предсказание → ветка «если предсказанный класс = Iris-setosa» → сообщение. Несколько входов во «Данные → Входы» по желанию.",
        studioPracticeKind: "empty",
        studioWorkspaceLevel: 1,
        goals: [
