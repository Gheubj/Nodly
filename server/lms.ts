    ...
    blocks: [
      {
        ...
        type: "text",
        textScale: "lg",
        body: `# Стажер Архива: дело Ирисов

Ты только что стал стажером Академии Noda. Твой ментор — Хранитель историй ИИ.

В первый же день происходит ЧП: в виртуальной оранжерее Архива распространяется вымышленная болезнь **«Серебряная пыльца»**. По архивной легенде, остановить ее может пигмент только одного вида ирисов, но после сбоя подписи цветов исчезли.

Хранителю нужна срочная помощь: быстро провести тебя через основы ИИ и научить модель отличать виды Iris по измерениям.`
      },
      {
        id: "q_story_1",
        type: "media",
        ...
        caption: "Хранитель: стажер, времени мало. Я объясню основы ИИ, а ты поможешь спасти оранжерею."
      },
      {
        id: "q_story_2",
        type: "text",
        body: `## Завязка для учителя

Хранитель открывает аварийный протокол стажера:

1. **Понять ИИ** — что это такое и где он уже работает.
2. **Разобрать данные** — почему без примеров модель слепа.
3. **Расследовать Iris** — найти признаки, по которым можно отличить виды цветов.
4. **Собрать модель в Noda** — восстановить классификатор для Архива.

Учитель объясняет материал как брифинг ментора. Ученики проходят проверки как стажеры, которые допускаются к следующей части дела.`
      },
...
- Несбалансированные классы могут сделать модель «предвзятой» к одному ответу.`
      ...
      { id: "q_div_3", type: "divider" },
      {
        ...
        type: "text",
        textScale: "lg",
        body: `## Сцена 4. Срочное дело: Ирисы

В архивной оранжерее три вида ирисов:
- **Iris setosa**
- **Iris versicolor**
- **Iris virginica**

Симуляция показывает: для остановки «Серебряной пыльцы» нужен пигмент из правильного вида. Но подписи исчезли, а вручную проверять сотни цветов слишком долго.

Остались измерения лепестков и чашелистиков. Значит, стажеру нужна модель классификации.`
      },
      {
        id: "q_iris_2",
        type: "text",
        body: `### Досье Iris dataset

Хранитель достает сохранившийся фрагмент базы:
...
Каждая строка — пример из архива. Числа — признаки. Вид — метка, то есть правильный ответ.`
      },
      {
        id: "q_iris_3",
        type: "text",
...
Хранитель подсказывает: у видов разные размеры лепестков.
...
Когда цель выполнена, ученик видит прогресс. Это превращает практику в понятную миссию.`
      ...
      { id: "q_div_8", type: "divider" },
      {
        ...
        type: "text",
        textScale: "lg",
        body: `## Финальная сцена. Аварийная лаборатория Noda

Теперь стажер допускается к лаборатории.

Задача: собрать в Noda мини-модель, которая определяет вид ириса по измерениям. Если модель справится, Хранитель сможет быстро найти нужные цветы и остановить «Серебряную пыльцу» в симуляции Архива.
...
Это первая настоящая разработка: не просто прочитать, а собрать цикл ИИ своими руками.`
      ...
      },
      {
        ...
        kind: "image",
        url: "/api/coach/success.png",
        caption: "Хранитель: хорошая работа, стажер. Архив Ирисов снова читается."
      },
      {
        id: "q_final_2",
        type: "text",
